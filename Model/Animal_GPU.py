...
model = Net(word_list, one_hot_dim, hidden_size).to('cuda')
model.load_state_dict(torch.load('../Param/Animal.pkl', map_location=torch.device('cuda')))
# loss = nn.CrossEntropyLoss()
# optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

# Dataset and DataLoader
ds_X = TensorDataset(X)
dl_X = DataLoader(ds_X, opt.batch_size, shuffle=False)
